...
describe("lineTranslate", () => {
  it("translates a line by an angle in degrees and a distance", () => {
    ...
    assert.deepEqual(geometric.lineTranslate(line, 0, 1), [
      [0, -1],
      [2, 1],
    ]);
    assert.deepEqual(geometric.lineTranslate(line, 0, -1), [
    ...
    assert.deepEqual(round(geometric.lineTranslate(line, 90, 1)), [
      [-1, 0],
      [1, 2],
    ]);
    assert.deepEqual(round(geometric.lineTranslate(line, 90, -1)), [
    ...
    assert.deepEqual(round(geometric.lineTranslate(line, 180, 1)), [
      [-2, -1],
      [0, 1],
    ]);
    assert.deepEqual(round(geometric.lineTranslate(line, 180, -1)), [
    ...
    assert.deepEqual(round(geometric.lineTranslate(line, 270, 1)), [
      [-1, -2],
      [1, 0],
    ]);
    assert.deepEqual(round(geometric.lineTranslate(line, 270, -1)), [
    ...
    assert.deepEqual(round(geometric.lineTranslate(line, 360, 1)), [
      [0, -1],
      [2, 1],
    ]);
    assert.deepEqual(round(geometric.lineTranslate(line, 360, -1)), [
